...
/**
 * This Agent plays the game of Hexifence in three stages: <br>
 * <ul>
 * <li>During the opening state, the agent greedily captures any available cells
 * and then makes a random safe move (one that does not offer any cells to the
 * oponent)</li>
 * <li>When there are only MIDGAME_SEARCH_DEPTH safe edges left, the agent will
 * begin to conduct an approximate adversarial search all the way to a terminal
 * state, considering only safe edges until none remain, and then considering
 * board features such as sacrifices, chains and loops. It will attemt to find a
 * winning move within a specified time, and then play that move.</li>
 * <li>Once the board has no more safe edges left, the agent will again search
 * until a terminal state considering sacrifices and other features looking for
 * a winning move, or a move that wil give the opponent a chance to make a
 * mistake if there are no winning moves left.</li>
 * </ul>
 * 
...
public class AgentFarrugiulian extends Agent {
   ...
   * 
   * @param piece
   *          Find a move given this this piece is to play next
   * @return SearchPair of an edge and the winning player assuming optimal play
   *         according to our search functions (which are not actually optimal
   *         as they are not considering all possible or equivalently possible
   *         moves, for example we do not consider sacrificing while safe edges
   *         remain)
   **/
  boolean timeout;
   ...
   * @return
   **/
  private SearchPair<Feature> featureSearch(FeatureSet features,
      int piece) {
    ...
    for (int i = 0; i < numIntersectedSacrifices; i++) {
      ...
      getIntersectedSacrifices(featuresTmp).get(i)
          .consume(Board.other(piece), false);

      SearchPair<Feature> pair = featureSearch(featuresTmp,
          Board.other(piece));
        ...
        if (shouldAdd) {
          for (Feature sacrifice : feature.getFeatures()) {
            if (sacrifice.length() < 3
                && !intersectedSacrifices.contains(sacrifice)) {
  ...
  private Feature getSmallestSacrifice(FeatureSet features) {
      ...
      if (feature.classification() == Feature.Classification.CHAIN
          ...
          || feature.classification() == Feature.Classification.LOOP
              && (feature.getFeatures().get(0).getFeatures().size() >= 3
                  || feature.getFeatures().get(0).getFeatures()
                      .get(0) != feature
                      && feature.getFeatures().get(0).getFeatures().get(0)
                          .classification() == Feature.Classification.LOOP
                  || feature.getFeatures().get(0).getFeatures()
                      .get(1) != feature
                      && feature.getFeatures().get(0).getFeatures().get(1)
                          .classification() == Feature.Classification.LOOP)
              && (smallestSacrifice == null
                  || feature.length() < smallestSacrifice.length())
    ...
    if (smallestSacrifice.length() > 3 || smallestSacrifice
        .classification() != Feature.Classification.ISO_LOOP
        && smallestSacrifice.length() > 2) {
  ...
  private int playout(FeatureSet features, int piece) {
    ...
    while (!features.isEmpty()) {
      ...
      if ((smallestSacrifice
          .classification() == Feature.Classification.CHAIN
          || smallestSacrifice
              .classification() == Feature.Classification.LOOP)
        ...
        Feature.Classification classification = smallestSacrifice
            .classification();
        ...
        if (numIsolatedClusters % 2 == 1
          ...
          int swing = classification == Feature.Classification.CHAIN ? 2
              : 4;
   ...
   * @return The piece expected to win
   **/
