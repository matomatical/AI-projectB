...
/**
 * This agent plays the Game of Hexifence with random capturing or safe moves up
 * to the point where sacrifices must be made, where it begins to attempt to
 * remain in control using a double-boxing strategy using a simple parity based
 * evaluation function.
 * 
 * This strategy works well if there are big chains and loops sacrificing on the
 * board which make sacrificing short chains worth it to get the long chains at
 * the end (more suitable for large board sizes) and also the parity evaluation
 * is not perfect as there are situations where the order in which chains are
 * sacrificed can effect the overall parity count and change who ends up in
 * control.
 * 
...
public class DoubleAgent extends Agent {
  ...
  public Edge getChoice() {
    ...
    if (freeScoring.size() > 0) {
      /*
       * System.out.println("Taking " + freeScoring.peek().i + "," +
       * freeScoring.peek().j); try { Thread.sleep(500); } catch
    ...
    if (scoring.size() > 0) {
       ...
       * System.out.println("Taking "+scoring.peek()); try { Thread.sleep(500);
      ...
      for (Edge edge : scoring) {
        ...
        if ((isLoop(edge) || !isLoop)
            && sacrificeSize(edge) < captureSize) {
          bestEdge = edge;
